...
it('compares our engine with reference', () => {
  ...
  for (const p of chart.planets) {
    ...
    console.log(
      `${p.planet.padEnd(10)} ${lonToStr(ourLon).padEnd(16)} ref:${lonToStr(ref.lon).padEnd(16)} Δ${diff.toFixed(2)}° H${p.house}${houseMatch} ${rxMatch}`,
    );
  }
  ...
  for (let i = 0; i < 12; i++) {
    ...
    console.log(
      `${label}  ${lonToStr(ours).padEnd(16)} ref:${lonToStr(ref).padEnd(16)} Δ${diff.toFixed(2)}°`,
    );
  }
