...
it('Hsinchu 1990-03-17 reference', () => {
  const chart = calculateNatalChart({
    year: 1990, month: 3, day: 17,
    hour: 2, minute: 51,
    latitude: 24.8, longitude: 120.967,
    locationName: '新竹市',
  ...
  for (const p of chart.planets) {
    ...
    console.log(`${p.planet}: ${deg}°${signs[signIdx]}${String(min).padStart(2,'0')}' lon=${lon.toFixed(2)} H${p.house} ${p.retrograde ? 'Rx' : ''}`);
  }
  ...
  for (const h of chart.houses) {
    ...
    const deg = Math.floor(lon % 30);
    const min = Math.round((lon % 30 - Math.floor(lon % 30)) * 60);
    console.log(`H${h.house}: ${deg}°${signs[signIdx]}${String(min).padStart(2,'0')}' lon=${lon.toFixed(2)}`);
  }
